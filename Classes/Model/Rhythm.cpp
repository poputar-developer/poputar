...
/*
 ...
 //创建信息节奏线
 Rhythm *rhythmLine = Rhythm::createRhythm(common);
 this->addChild(rhythmLine);
 rhythmLine->setTag(currentBeat);
 ...
 //获得开始时间需要的距离
 scanf("%f%f",&startWidth,&visibleWidth);
 float startDistance =(int)startWidth%(int)visibleWidth;
 
 //获得开始创建的位置
 float createWidth = rhythm_begin_distance - startDistance;
 
 time = common->startTime;
 
 rhythmLine->setPosition(Vec2(createWidth,0));
 
 }else{
 rhythmLine->setPosition(Vec2(0,0));
 time = rhythm_begin_time;
 }
 
 MoveTo *beginMove = MoveTo::create(time,Vec2(rhythm_begin_distance,0));
 Sequence *sequence = Sequence::create(beginMove,CallFunc::create([this,rhythmLine](){
 Size visibleSize = Director::getInstance()->getVisibleSize();
 int visibleWidth = visibleSize.width;
 
 float endSpeed=common->unitSpeed*addedUint;
 
 MoveTo *endMove = MoveTo::create(endSpeed, Vec2(visibleWidth,0));
 FadeOut *out = FadeOut::create(endSpeed);
 Spawn *spawn = Spawn::create(endMove,out,NULL);
 Sequence *removeSq=Sequence::create(spawn,CallFunc::create([this](){
